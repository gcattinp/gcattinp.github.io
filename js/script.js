window.onload = function() {
  ...
  const bannerBera = document.querySelector('.banner-beras');
  const headerBera = document.querySelector('.header-beras');
  const beraFolder = document.querySelector('#bera-folder');
  const beraNftsContainer = document.querySelector('#beraNftsContainer');
  const closeBeraNftsButton = document.querySelector('#closeBeraNftsBtn');
  ...
  jpegsFolder.addEventListener('click', (event) => {
    if (!isJpegsVisible) {
      event.stopPropagation();
  ...
  remiliaFolder.addEventListener('click', (event) => {
  ...
  // For bera folder
  headerBera.addEventListener('mousedown', (e) => {
    mouseDown = true;
    startX = e.clientX;
    startY = e.clientY;
    initialLeft = bannerBera.offsetLeft;
    initialTop = bannerBera.offsetTop;
    currentDragFunction = calculatePosition(bannerBera);
    window.addEventListener('mousemove', currentDragFunction);
  });
  ...
  window.addEventListener('mouseup', () => {
    ...
    if (currentDragFunction) {
        ...
        currentDragFunction = null;
    }
  });

  beraFolder.addEventListener('click', (event) => {
    if (!isBeraNftsVisible) {
      event.stopPropagation();
      beraNftsContainer.style.display = "block";
    } else {
      beraNftsContainer.style.display = "none";
    }
    isBeraNftsVisible = !isBeraNftsVisible;
  });


  closeBeraNftsButton.addEventListener('click', () => {
    beraNftsContainer.style.display = "none";
    isBeraNftsVisible = false;
  });
};
